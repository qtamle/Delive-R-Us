...
namespace SMPScripts
{
    public class MotoAnimController : MonoBehaviour
    {
        ...
        void Start()
        {
            motoController = FindFirstObjectByType<MotoController>();
            motoStatus = FindFirstObjectByType<MotoStatus>();
        ...
        void Update()
        {
            ...
            if (motoStatus != null)
            {
                ...
                {
                    if  (anim != null && !motoController.isAirborne && motoStatus.onBike)
                    {
            ...
            {
                if (anim != null && !motoController.isAirborne)
                {
                    clipInfoCurrent = anim.GetCurrentAnimatorClipInfo(0)[0].clip.name;
                    if (clipInfoCurrent == "MotoIdleToStart" && clipInfoLast == "MotoIdle")
                        StartCoroutine(LeftFootIK(0));
