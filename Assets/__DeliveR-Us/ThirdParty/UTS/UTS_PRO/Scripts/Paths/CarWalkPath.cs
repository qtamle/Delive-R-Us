...
public class CarWalkPath : WalkPath
{
    ...
    public override void SpawnOnePeople(int w, bool forward)
    {
        ...
        Vector3 targetPos = !forward ? points[w, pointLength[0] - 2] : points[w, 1];

        if (IsSpawnPointValid(targetPos))
        {
            if (!forward)
            {
    ...
    public override void SpawnPeople()
    {
        ...
        for (int wayIndex = 0; wayIndex < numberOfWays; wayIndex++)
        {
            ...
            for (int peopleIndex = 0; peopleIndex < peopleCount; peopleIndex++)
            {
                ...
                if (!IsSpawnPointValid(routePosition))
                    continue;
                ...
                //StartCoroutine(DestroyIfStuck(people));
            }
        }
    }
    ...
    private bool IsSpawnPointValid(Vector3 position, float minDistance = 20f, float maxDistance = 250f)
    {
        Camera cam = Camera.main;
        if (!cam) return false;

        Vector3 camPos = cam.transform.position;
        Vector3 dirToSpawn = position - camPos;
        float distance = dirToSpawn.magnitude;

        // ❌ Reject if too close or too far
        if (distance < minDistance || distance > maxDistance)
            return false;

        // ❌ Reject if in front of the camera (angle too small)
        float angle = Vector3.Angle(cam.transform.forward, dirToSpawn);
        if (angle < 100f) // means it's within ~100° front view cone
            return false;

        // ❌ Reject if it's currently visible on screen
        ...
        // ❌ Reject if no obstacle in between (i.e., it's in line of sight)
        if (!Physics.Raycast(camPos, dirToSpawn.normalized, out RaycastHit hit, distance))
            return false;

        // ✅ Passed all checks — safe to spawn
        return true;
    }



    //private IEnumerator DestroyIfStuck(GameObject car)
    //{
    ...
    //    // If car is still inside ground or overlapping
    //    if (Physics.CheckSphere(pos, 1f, LayerMask.GetMask("Default")) || pos.y < -5f)
    //    {
    //        Destroy(car);
    //    }
    //}
