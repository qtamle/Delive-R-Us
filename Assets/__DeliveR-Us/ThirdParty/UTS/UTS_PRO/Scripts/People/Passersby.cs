...
public class Passersby : MonoBehaviour
{
    ...
    [Tooltip(
        "Layers of car, traffic light, pedestrians, player / Слои автомобиля, светофора, пешеходов, игрока"
    )]
    ...
    private float _customWalkAnimationMultiplier = 1.0f;

    [SerializeField]
    [Tooltip("Running animation speed / Скорость анимации бега")]
    private float _customRunAnimationMultiplier = 1.0f;
    ...
    private void Update()
    {
        ...
        {
            animator.Play(Animator.StringToHash(animationState.ToString()));
        }
    ...
    {
        Vector3 randFinishPos = new Vector3(
            movePath.finishPos.x + movePath.randXFinish,
            movePath.finishPos.y,
            movePath.finishPos.z + movePath.randZFinish
        );

        Vector3 targetPos = new Vector3(
            randFinishPos.x,
            rigBody.transform.position.y,
            randFinishPos.z
        );

        var richPointDistance = Vector3.Distance(
            Vector3.ProjectOnPlane(rigBody.transform.position, Vector3.up),
            Vector3.ProjectOnPlane(randFinishPos, Vector3.up)
        );

        if (
            richPointDistance < 0.2f
        ...
        )
        {
            if (movePath.forward)
            {
                if (movePath.targetPoint < movePath.targetPointsTotal)
                    targetPos = movePath.walkPath.getNextPoint(
                        movePath.w,
                        movePath.targetPoint + 1
                    );
                else
        ...
        else if (richPointDistance <= movePath._walkPointThreshold && movePath.forward)
        {
            if (movePath.targetPoint != movePath.targetPointsTotal)
            {
                ...
                movePath.finishPos = movePath.walkPath.getNextPoint(
                    movePath.w,
                    movePath.targetPoint
                );
            }
            else if (movePath.targetPoint == movePath.targetPointsTotal)
            {
                ...
                else
                {
                    ...
                    {
                        movePath
                            .walkPath.SpawnPoints[movePath.w]
                            .AddToSpawnQuery(new MovePathParams { });
                    }
    ...
    private void SortTargetsInView<T>(IList<T> list, Collider[] targetsInViewRadius, string tag)
    {
        ...
        {
            if (!c.transform.CompareTag(tag) || c.transform.position == transform.position)
                continue;

            if (typeof(T) == typeof(Passersby))
            {
                if (
                    c.transform.parent.name != transform.parent.name
                    || c.GetComponent<MovePath>().w != movePath.w
                )
                    continue;
            }
        ...
        foreach (T t in list)
        {
            ...
            if (Vector3.Angle(transform.forward, dirToTarget) < viewAngle / 2)
            {
                ...
                if (typeof(T) == typeof(Passersby))
                {
                    ...
                    {
                        dstToNearTarget = Vector3.Distance(
                            transform.position,
                            nearPassersby.transform.position
                        );
                    }
                }
                else if (typeof(T) == typeof(SemaphorePeople))
                {
                    if (nearSemaphore != null)
                    {
                        dstToNearTarget = Vector3.Distance(
                            transform.position,
                            nearSemaphore.transform.position
                        );
                    }
                }
                else if (typeof(T) == typeof(Transform))
    ...
    private void ActionNearPassersby()
    {
        if (nearPassersby != null)
        {
            ...
            if (
                animationState == AnimationState.idle1
                && nearPassersby.ANIMATION_STATE != AnimationState.idle1
            )
            {
                if (!timeToWalk)
                {
